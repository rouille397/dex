...
export const transformBetResponse = (betResponse: BetResponse): Bet => {
  const bet = {
    ...
    user: {
      ...
      totalCFX: numberOrNull(betResponse.user.totalCFX),
    },
  } as Bet
...
export const transformTotalWonResponse = (
  ...
  const totalCFX = marketResponse.totalCFX ? parseFloat(marketResponse.totalCFX) : 0
  const totalCFXTreasury = marketResponse.totalCFXTreasury ? parseFloat(marketResponse.totalCFXTreasury) : 0

  return Math.max(totalCFX - (totalCFXTreasury + houseRounds), 0)
}
